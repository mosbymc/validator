...
The html page 'validate.html' is my test page for the validator to do initial testing of new features. You'll find it's fairly scatter-brained, but I believe all of the functionality of validator.js is on display there. Note that the time delay in some of the custom rules as well as the function called before validation on some of the forms is due to a timeout placed in the function to simulate an asynchrous function, and not due to the validator itself. To get validate.html working, you'll need the files form.css and validator.js. It's also currently using JQuery and JQuery UI CDNs. Please not that while the example page requires JQuery UI, the validator itself does not.
